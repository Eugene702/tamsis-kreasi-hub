...
const NavbarClient = ({ categories, userPhoto }: NavbarClientProps) => {
    ...
    const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        if (searchQuery.trim()) {
            router.push(`/artwork?search=${encodeURIComponent(searchQuery.trim())}`)
            setSearchQuery('')
            setMobileOpen(false)
        } else {
            router.push('/artwork')
    ...
    return (
        <header className="sticky top-0 z-40">
            <nav className="px-5 md:px-10 h-16 flex items-center gap-6 backdrop-blur bg-base-100/70 border-b border-base-300/60 supports-[backdrop-filter]:bg-base-100/55">
                ...
                {showSearch && (
                    <form className="hidden md:block flex-1 max-w-xl" onSubmit={handleSearch}>
                        <div className="join w-full input input-sm md:input-md !rounded-full items-center px-2 pr-3 gap-1 bg-base-100/80">
                            <input
                                ...
                                aria-label="Cari"
                            />
                            <button type="submit" className="btn bg-emerald-700 hover:bg-emerald-600 btn-circle btn-sm text-white" aria-label="Cari">
