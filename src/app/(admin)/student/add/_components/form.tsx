...
const Form = () => {
    ...
    const schema = object().shape({
        ...
        confirmPassword: string().oneOf([ref("password"), undefined], "Kata sandi tidak sesuai").required("Ulangi kata sandi wajib diisi!"),
    })
    ...
    useEffect(() => {
        if (values.photo && !errors.photo) {
            setTempPhoto(URL.createObjectURL(values.photo))
        }else{
            setTempPhoto(null)
        }
    }, [values.photo])
    ...
    return <form onSubmit={handleSubmit}>
        <div className="flex justify-center items-center flex-col">
            ...
            { (!errors.photo && values.photo) && <button type="button" className="btn btn-outline !rounded-xl w-fit mt-5" onClick={handleRemovePhoto}>Hapus Foto</button> }
